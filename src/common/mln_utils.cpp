...
int socketpair(int domain, int type, int protocol, int sv[2]) { return pipe(sv); }
#endif
...
static inline unsigned long xchg(void *ptr, unsigned long x) {
  __asm__ __volatile__("xchg %0, %1" : "=r"(x) : "m"(*(volatile long *)ptr), "0"(x) : "memory");
  return x;
}
...
int spin_trylock(void *lock) { return !xchg(lock, 1); }
#endif
