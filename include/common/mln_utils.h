...
#define mln_container_of(ptr, type, member)                                  \
  ({                                                                         \
    type *__rptr = NULL;                                                     \
    if ((ptr) != NULL) {                                                     \
      __rptr = (type *)((char *)((const typeof(((type *)0)->member) *)(ptr)) \
                        - mln_offsetof(type, member));                       \
    }                                                                        \
    __rptr;                                                                  \
  })
...
#  define __NONNULL5(v, w, x, y, z) \
    __nonnull((v)) __nonnull((w)) __nonnull((x)) __nonnull((y)) __nonnull((z))
#endif
...
#  define mln_spin_init(lock_ptr) pthread_spin_init((lock_ptr), PTHREAD_PROCESS_PRIVATE)
#  define mln_spin_destroy(lock_ptr) pthread_spin_destroy((lock_ptr))
