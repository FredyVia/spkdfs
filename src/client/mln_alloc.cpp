...
MLN_CHAIN_FUNC_DECLARE(mln_alloc_shm, mln_alloc_shm_t, static inline void, );
...
static inline int mln_alloc_shm_allowed(mln_alloc_shm_t *as, mln_off_t *Boff, mln_off_t *boff,
                                        mln_size_t size);
static inline void *mln_alloc_shm_set_bitmap(mln_alloc_shm_t *as, mln_off_t Boff, mln_off_t boff,
                                             mln_size_t size);
static inline mln_alloc_shm_t *mln_alloc_shm_new_block(mln_alloc_t *pool, mln_off_t *Boff,
                                                       mln_off_t *boff, mln_size_t size);
...
static inline mln_alloc_shm_t *mln_alloc_shm_new(mln_alloc_t *pool, mln_size_t size, int is_large) {
  ...
  mln_u8ptr_t p = (mln_u8ptr_t)pool->mem + sizeof(mln_alloc_t);

  for (tmp = pool->shm_head; tmp != NULL; tmp = tmp->next) {
    if ((mln_u8ptr_t)(tmp->addr) - p >= size) break;
    p = (mln_u8ptr_t)tmp->addr + tmp->size;
  }
...
MLN_FUNC(mln_alloc_t *, mln_alloc_shm_init, (struct mln_alloc_shm_attr_s * attr), (attr), {
  ...
  if (attr->size < M_ALLOC_SHM_DEFAULT_SIZE + 1024 || attr->locker == NULL || attr->lock == NULL
      || attr->unlock == NULL) {
    return NULL;
  }

#if defined(WIN32)
  if ((handle = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE,
#  if defined(__x86_64)
                                  (u_long)(attr->size >> 32),
#  else
                                    0,
#  endif
                                  (u_long)(attr->size & 0xffffffff), NULL))
      == NULL) {
    return NULL;
  }
...
MLN_FUNC_VOID(void, mln_alloc_destroy, (mln_alloc_t * pool), (pool), {
  ...
  if (parent != NULL && mln_alloc_is_shm(parent))
    if (parent->lock(parent->locker) != 0) return;
  if (pool->mem == NULL) {
...
#  endif
  ...
  if (parent != NULL && mln_alloc_is_shm(parent)) (void)parent->unlock(parent->locker);
})
...
static inline mln_alloc_mgr_t *mln_alloc_get_mgr_by_size(mln_alloc_mgr_t *tbl, mln_size_t size) {
  if (size > tbl[M_ALLOC_MGR_LEN - 1].blk_size) return NULL;
...
#else
    mln_u8ptr_t ptr = (mln_u8ptr_t)mln_alloc_m(pool, size);
    if (ptr == NULL) return NULL;
    memset(ptr, 0, size);
    return ptr;
#endif
...
MLN_FUNC(void *, mln_alloc_re, (mln_alloc_t * pool, void *ptr, mln_size_t size), (pool, ptr, size),
    ...
    mln_u8ptr_t new_ptr = (mln_u8ptr_t)mln_alloc_m(pool, size);
...
static inline void *mln_alloc_shm_large_m(mln_alloc_t *pool, mln_size_t size) {
  ...
  if ((as = mln_alloc_shm_new(pool, size + sizeof(mln_alloc_shm_t) + sizeof(mln_alloc_blk_t), 1))
      == NULL)
    return NULL;
  as->nfree = 0;
...
static inline void *mln_alloc_shm_set_bitmap(mln_alloc_shm_t *as, mln_off_t Boff, mln_off_t boff,
  ...
  addr = (mln_u8ptr_t)as->addr + (Boff * 8 + (7 - boff)) * M_ALLOC_SHM_BIT_SIZE;
 ...
 * chain
 */
MLN_CHAIN_FUNC_DEFINE(mln_blk, mln_alloc_blk_t, static inline void, prev, next);
MLN_CHAIN_FUNC_DEFINE(mln_chunk, mln_alloc_chunk_t, static inline void, prev, next);
MLN_CHAIN_FUNC_DEFINE(mln_alloc_shm, mln_alloc_shm_t, static inline void, prev, next);
